      ...
      <div className="sticky top-0 z-50 bg-midnight w-full backdrop-blur supports-[backdrop-filter]:bg-midnight/65">
        <div className="flex h-14 justify-between barwidth:justify-center items-center">
          ...
          <div className="flex mr-2.5 small:mr-4.5 barwidth:mr-0">
            ...
            <a className="px-1.5 py-1 ml-2 rounded-md hover:bg-lightmidnight transition-colors duration-200 cursor-pointer navwidth:hidden">
              <CgMenuRightAlt className="text-offwhite h-8 w-8 navwidth:hidden" onClick={() => { setShowMenu(!showMenu); setPressedMenu(true) }}/>
            </a>
          </div>
        </div>
        <div className="w-full h-px bg-lightmidnight"></div>
      </div>
      <div className={`h-navwidth:hidden absolute text-offwhite overscroll-contain z-10 w-full h-[calc(100%-57px)] bg-midnight backdrop-blur supports-[backdrop-filter]:bg-midnight/65 
